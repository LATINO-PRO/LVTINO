...
try:
    from causvid.models.wan.bidirectional_LVTINO_VIDUE_optuna import PipelineLVTINO  # project import
except Exception:
...
def suggest_hparams(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    hp["latino_cg_epsilon"] = trial.suggest_categorical("latino_cg_epsilon", [0.0, 1e-8, 1e-6, 1e-4, 1e-2])
...
def build_cache_for_video(
    ...
    vid_dir_sparse = os.path.join(interp_input_root, f"{vid_id:05d}")
...
def run_one_cached(
    ...
    if save_outputs:
        export_to_video(restored.permute(0, 2, 3, 1).detach().cpu().numpy(), os.path.join(out_dir, "output.mp4"), fps=fps)
...
def main():
    ...
    pipe_img = DiffusionPipeline.from_pretrained(base_model_id, unet=unet, vae=vae, torch_dtype=torch.float16, variant="fp16", guidance_scale=0).to(device)
    pipe_img.scheduler = LCMScheduler.from_config(pipe_img.scheduler.config)

    prompt_for_embed = "a high resolution photo"
    text_embeddings, _, pooled_text_embeds, _ = pipe_img.encode_prompt(prompt_for_embed, device=device, num_images_per_prompt=1, do_classifier_free_guidance=False)

    image_height, image_width = 768, 1280
    time_ids = pipe_img._get_add_time_ids(original_size=(image_height, image_width), crops_coords_top_left=(0, 0), target_size=(image_height, image_width), dtype=torch.float16, text_encoder_projection_dim=1280).to(device)
    ...
    forward_model = TemporalSRThenSROp(factor_t=args.temporal_factor, factor=args.sr_factor, filter="bicubic", padding="reflect")
    ...
    if args.optuna:
        ...
        for vid in tune_ids:
            caches.append(build_cache_for_video(
                vid_id=vid,
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            for cache in caches:
                ...
                cache_local["out_dir"] = os.path.join(args.output_folder, "_optuna_tmp", f"trial_{trial.number:04d}", f"{vid:05d}")
    ...
    # Full run (uses pipe.hparams if set)
    for vid_id in tqdm(video_ids, desc="Processing videos"):
        ...
        x_init = torch.stack([torch.from_numpy(im).permute(2, 0, 1).to(device=device, dtype=work_dtype) / 255.0 for im in x_init_frames], dim=0)
        ...
        x_init_up = F.interpolate(x_init.to(dtype=torch.float32), size=(x_gt.shape[2], x_gt.shape[3]), mode="bilinear", align_corners=False)
        ...
        z0_hat = pipe.vae.encode_from_pixel(x_init_lat)
        next_timestep = pipe.denoising_step_list[0] * torch.ones(z0_hat.shape[:2], dtype=torch.long, device=z0_hat.device)
        noisy_image_or_video = pipe.scheduler.add_noise(z0_hat.flatten(0, 1), torch.randn_like(z0_hat.flatten(0, 1)), next_timestep.flatten(0, 1)).unflatten(0, z0_hat.shape[:2])
        ...
        restored = out[0]
        export_to_video(restored.permute(0, 2, 3, 1).detach().cpu().numpy(), os.path.join(out_dir, "output.mp4"), fps=args.fps)
