                ...
                for i in range(len(input_seqs)):   #, bm_seq, label_seq   , bm_seqs, label_seqs
                    ...
                    for j in range(self.n_outputs):
                        ...
                        if self.args.default_data == 'Adobe':
                            save_name = os.path.join(gt_frames_path, '%04d.jpg' % (
                                        int(filename) + j))
                            print(save_name)
                        else:
                            save_name = os.path.join(gt_frames_path, '%06d.jpg' % (int(filename)-(self.m//2)+j))
                        '''
                        gt = imageio.imread(save_name)
                        gt = cv2.resize(gt, None, fx=output_img.shape[1]/gt.shape[1], fy=output_img.shape[0]/gt.shape[0], interpolation=cv2.INTER_CUBIC)
