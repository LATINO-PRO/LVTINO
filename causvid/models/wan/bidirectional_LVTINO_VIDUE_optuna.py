...
# -------------------------
# Main pipeline (SR8x8 + VIDUE init)
# -------------------------
class PipelineLVTINO(torch.nn.Module):
    """
    Pipeline used by LVTINO_VIDUE.py.

    Optuna support:
      - set `pipe.hparams = {...}` before calling `inference()`
      - keys (all optional, with defaults):
          tv_lam_hi, tv_lam_lo, tv_lam_switch
          tv_iters, tv_lr
          final_prox_gamma
          latino_cg_steps, latino_cg_epsilon
    ...
    def __init__(self, args, device):
        ...
        # Optuna-injectable hyperparameters
    ...
    def inference(
        ...
        output_folder: str,
        fps: int,
        save_intermediates: Optional[bool] = None,
    ) -> torch.Tensor:

        hp = self.hparams or {}
        if save_intermediates is None:
            save_intermediates = bool(hp.get("save_intermediates", True))
        ...
        # --- Hyperparams
        tv_lam_switch = int(hp.get("tv_lam_switch", 2))
        tv_lam_hi = float(hp.get("tv_lam_hi", 1e-5))
        tv_lam_lo = float(hp.get("tv_lam_lo", 1e-6))
        tv_iters = int(hp.get("tv_iters", 100))
        tv_lr = float(hp.get("tv_lr", 5e-3))

        latino_cg_steps = int(hp.get("latino_cg_steps", 5))
        latino_cg_epsilon = float(hp.get("latino_cg_epsilon", 0.0))
        ...
        for index, current_timestep in enumerate(self.denoising_step_list):
            ...
            if index > 0:
                ...
                x0_LATINO = LATINO_img(
                    ...
                    index=index,
                    forward_model=forward_model,
                    dc_cg_steps=latino_cg_steps,
                    dc_epsilon=latino_cg_epsilon,
                )
            ...
            lam = tv_lam_hi if index < tv_lam_switch else tv_lam_lo
