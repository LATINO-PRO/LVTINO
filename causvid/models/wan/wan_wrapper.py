...
class WanVAEWrapper(VAEInterface):
    ...
    def encode_from_pixel(self, video: torch.Tensor) -> torch.Tensor:
        """
        video: pixel video in [-1,1], shape [B, F, 3, H, W]
        returns: latent tensor [B, F, 16, H, W]
        """
        # [B, F, 3, H, W] -> [B, 3, F, H, W]
        xs = video.permute(0, 2, 1, 3, 4)

        # --- make input & scale match the VAE weights ---
        # prefer buffers first, else parameters
        dtype_model, device_model = None, None
        for b in self.model.buffers():
            dtype_model, device_model = b.dtype, b.device
            break
        if dtype_model is None:
            p = next(self.model.parameters())
            dtype_model, device_model = p.dtype, p.device

        mean = self.mean.to(device=device_model, dtype=dtype_model)
        inv_std = (1.0 / self.std).to(device=device_model, dtype=dtype_model)
        scale = [mean, inv_std]  # flip to [mean, std] if your VAE expects that

        latents = []
        for x in xs:  # x: [3, F, H, W] for one batch item
            xb = x.unsqueeze(0).to(device=device_model, dtype=dtype_model)   # NO .float()
            z = self.model.encode(xb, scale).squeeze(0)  # [16, F, H, W], dtype = dtype_model
            latents.append(z)

        z_all = torch.stack(latents, dim=0)  # [B, 16, F, H, W]
        return z_all.permute(0, 2, 1, 3, 4)  # [B, F, 16, H, W]
...
class WanDiffusionWrapper(DiffusionModelInterface):
    def __init__(self):
        ...
        self.seq_len = 80640  # 32760 for [1, 21, 16, 60, 104]
        super().post_init()
